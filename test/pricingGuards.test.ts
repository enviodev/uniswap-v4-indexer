/**
 * Tests for the two pricing guards that keep junk USD numbers out of the data:
 *
 * 1. findNativePerToken's imbalance guard (MAX_PRICING_POOL_VALUE_IMBALANCE):
 *    a pool may only set a token's derivedETH when the value it implies for the
 *    token side is ≤ 100× the pool's verifiable (whitelisted) side.
 *
 * 2. getTrackedTVLUSD: pool TVL bounded by verifiable capital (v2-subgraph
...
describe("getTrackedTVLUSD", () => {
  const weth = makeToken(WETH, { derivedETH: bd(1) });
  const usdc = makeToken(USDC, { derivedETH: bd("0.0005") });

  it("sums both sides when both tokens are whitelisted", () => {
    const tracked = getTrackedTVLUSD(
      bd("100"), // 100 WETH = $200k
      weth,
      bd("200000"), // 200k USDC = $200k
      usdc,
      WHITELIST,
      ETH_PRICE_USD
    );
    expect(tracked.eq(bd("400000"))).toBe(true);
  });

  it("doubles the whitelisted side when only one token is whitelisted", () => {
    // Balanced memecoin pool: 2× the real side ≈ the true total.
    const pepe = makeToken("0x6982508145454ce325ddbe47a25d4ec3d2311933", {
      derivedETH: bd("5.88e-9"),
    });
    const tracked = getTrackedTVLUSD(
      bd("500"),
      weth,
    ...
    expect(tracked.eq(bd("2000000"))).toBe(true); // 2 × 500 ETH × $2000
  });

  it("bounds the honest-price fake-balance attack (ETH/1xETH anatomy)", () => {
    // Even if the imbalance guard were somehow bypassed and 1xETH kept
    // derivedETH = 1, tracked TVL stays 2 × the real 1.01 ETH — not $2.3e12.
    const oneXEth = makeToken("0x41d91195960719e3c0077a9dd1716a050708b9b1", {
      derivedETH: bd(1),
    });
    const tracked = getTrackedTVLUSD(
    ...
    expect(tracked.eq(bd("4040"))).toBe(true); // 2 × 1.01 × $2000
  });

  it("bounds the USDC/being anatomy to 2× the real USDC", () => {
    const being = makeToken("0xbbbe40e7ae6e22aad49d6a7c9389ef25714be179", {
      derivedETH: bd("1.25e16"), // poisoned (pre-guard historical value)
    });
    const tracked = getTrackedTVLUSD(
      bd("4388.723262"),
      usdc,
    ...
    // 2 × 4388.723262 USDC × 0.0005 ETH × $2000 = $8,777.446524
    expect(tracked.eq(bd("8777.446524"))).toBe(true);
  });

  it("returns 0 when neither token is whitelisted (transitive contamination)", () => {
    // BLOTIX/SAFEMONEY anatomy: junk-priced token in a pool with no
    // whitelisted side must contribute nothing.
    const blotix = makeToken("0x3333333333333333333333333333333333333333", {
      derivedETH: bd("0.001"),
    });
    const safemoney = makeToken("0xc4abeddacfa65e25d6c942861fb3bdc9c00eeb13", {
      derivedETH: bd("0.0011495"), // poisoned elsewhere
    });
    const tracked = getTrackedTVLUSD(
      bd("1000000"),
      blotix,
    ...
    expect(tracked.eq(bd(0))).toBe(true);
  });
});
